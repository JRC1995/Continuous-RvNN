...
dev_keys = ["normal"]
test_keys = [str(i) for i in range(13)]
np.random.seed(SEED)
random.seed(SEED)
...
for key in test_keys:
    test_path[key] = Path('../data/NLI_data/PNLI/test{}'.format(key))


Path('processed_data/').mkdir(parents=True, exist_ok=True)

train_save_path = Path('processed_data/PNLI_LG_train.jsonl')
dev_save_path = {}
for key in dev_keys:
    dev_save_path[key] = Path('processed_data/PNLI_LG_dev_{}.jsonl'.format(key))
test_save_path = {}
for key in test_keys:
    test_save_path[key] = Path('processed_data/PNLI_LG_test_{}.jsonl'.format(key))
metadata_save_path = fspath(Path("processed_data/PNLI_LG_metadata.pkl"))
...
for key in test_keys:
    test_sequences1[key], \
        test_sequences2[key], \
        test_labels[key] = process_data(test_path[key], update_vocab=False)
